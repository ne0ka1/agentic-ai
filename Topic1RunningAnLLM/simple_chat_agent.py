...
# Maximum number of messages to keep in history (including system message)
MAX_HISTORY_MESSAGES = 10

# Option to turn off conversation history
USE_CONVERSATION_HISTORY = False
if USE_CONVERSATION_HISTORY:
    print("Conversation history is enabled.")
else:
    print("Conversation history is disabled.")
...
while True:
    ...
    # Keep a fixed window of history while preserving the system prompt.
    # When the window is exceeded, we drop the oldest non-system messages
    # but always keep chat_history[0] (the system message).
    if len(chat_history) > MAX_HISTORY_MESSAGES:
        chat_history = [
            chat_history[0],
            *chat_history[-(MAX_HISTORY_MESSAGES - 1):],
        ]
