...
#
# Multi-agent chat (Llama & Qwen) with checkpointing and crash recovery.
# The graph is compiled with a checkpointer and a fixed thread_id so state
# is saved after every step. Kill the process (e.g. Ctrl+C) and restart to resume.
# For recovery across restarts: pip install langgraph-checkpoint-sqlite
...
def create_graph(models: dict, checkpointer):
    """Create the LangGraph state graph with natural model switching and checkpointing."""
...
def main():
    print("=" * 50)
    print("Multi-Agent LangGraph (Llama & Qwen) — with checkpointing")
    print("=" * 50)
    print()

    # Prefer SQLite on disk (survives process restarts); fall back to in-memory.
    try:
        ...
        print(f"[Checkpoint] Using SQLite: {db_path} (state survives process restart)")
    except ImportError:
        from langgraph.checkpoint.memory import MemorySaver

        cm = nullcontext(MemorySaver())
        print(
            "[Checkpoint] Using in-memory (install langgraph-checkpoint-sqlite "
            "for kill-and-restart recovery)"
        )
    ...
    with cm as checkpointer:
        ...
        print("\nCreating LangGraph...")
        graph = create_graph(models, checkpointer)
        print("Graph created successfully!")

        # Fixed thread_id: all runs share the same conversation and checkpoint chain.
        THREAD_ID = "main"
        ...
        # Crash recovery: if we have a checkpoint (e.g. after restart), resume from it.
        state_snapshot = graph.get_state(config)
        has_checkpoint = (
            state_snapshot.values
            and (state_snapshot.values.get("messages") or state_snapshot.next)
        )
        if has_checkpoint:
            print("[Resuming from last checkpoint — nothing lost from previous run.]\n")
