...
def create_llm(model_id="meta-llama/Llama-3.2-1B-Instruct"):
    ...
    model_id is model identifier on HuggingFace Hub
    """
...
def create_graph(llama, qwen):
    ...
    def get_user_input(state: AgentState) -> dict:
        ...
        # check verbosity
        if user_input.lower() in ['verbose']:
            return {
                "user_input": user_input,
                "verbose": True
            }
    ...
    #   - llama_response: Set to the LLM's generated response
    def call_llama(state: AgentState) -> dict:
        ...
        Updates state:
            - llama_response: The text generated by the LLM
        """
    ...
    #   - user_input: Unchanged (read only)
    #   - should_continue: Unchanged (read only)
    #   - qwen_response: Set to the LLM's generated response
    def call_qwen(state: AgentState) -> dict:
        ...
        Reads state:
            - user_input: The text to send to the LLM
        Updates state:
            - qwen_response: The text generated by the LLM
        """
        user_input = state["user_input"]
...
def main():
    ...
    llama = create_llm(model_id="meta-llama/Llama-3.2-1B-Instruct")
    qwen = create_llm(model_id="Qwen/Qwen2.5-0.5B-Instruct")
